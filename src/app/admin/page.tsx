...
export default function AdminControlPanel() {
  ...
  useEffect(() => {
    try {
      const savedDark = localStorage.getItem("thumbstop_theme_is_dark");
      if (savedDark !== null) {
      ...
      if (savedThemeConfig) {
        ...
        if (parsed.isDark !== undefined && savedDark === null) setIsDark(parsed.isDark);
  ...
  const saveManagementThemeConfig = (
    newThemeId?: string,
    newAccent?: string,
    newStyle?: "glass" | "solid" | "bordered",
    newGlow?: "subtle" | "vibrant" | "off",
    newIsDark?: boolean
  ) => {
    ...
    const dMode = newIsDark !== undefined ? newIsDark : isDark;
    ...
    if (newIsDark !== undefined) setIsDark(newIsDark);

    try {
      localStorage.setItem("thumbstop_theme_is_dark", String(dMode));
          ...
          <div className={`sticky top-0 z-30 w-full backdrop-blur-md transition-colors ${theme.headerBg} border-b`}>
            ...
            <header className="h-16 px-4 sm:px-6 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2.5">
                ...
                {/* 🎨 Quick Theme Selector Popover & Dark/Light Toggle */}
                <div className="relative">
                  <button
                    onClick={() => setIsThemeMenuOpen(!isThemeMenuOpen)}
                    className={`px-2.5 py-1.5 rounded-lg text-xs flex items-center gap-1.5 border transition-all ${
                      isThemeMenuOpen
                        ? "bg-cyan-500/20 border-cyan-500/50 text-cyan-300 shadow-md shadow-cyan-500/20 ring-1 ring-cyan-500/40"
                        : isDark
                        ? "bg-[#091322] border-white/10 text-cyan-400 hover:text-cyan-300 hover:bg-white/5"
                        : "bg-white border-slate-200 text-slate-700 hover:bg-slate-100 shadow-sm"
                    }`}
                    title="Change Theme & Lighting (থিম ও লাইটিং পরিবর্তন)"
                  >
                    <Palette className="w-4 h-4 text-cyan-400" />
                    <span className="hidden md:inline text-[11px] font-semibold">{activeMgmtTheme.name.split(" ")[0]}</span>
                  ...
                  {isThemeMenuOpen && (
                    <>
                      ...
                      />
                      <div
                        className={`absolute right-0 mt-2 w-72 sm:w-80 rounded-2xl p-3 shadow-2xl border z-50 animate-in zoom-in-95 duration-150 ${
                          isDark
                            ? "bg-[#08101E]/95 backdrop-blur-xl border-cyan-500/30 text-white shadow-cyan-950/50"
                            : "bg-white/95 backdrop-blur-xl border-slate-200 text-slate-900 shadow-slate-300/50"
                        }`}
                      >
                        {/* Header & Quick Mode Switch */}
                        <div className={`flex items-center justify-between pb-2.5 mb-2.5 border-b ${isDark ? "border-white/10" : "border-slate-200"}`}>
                          <div>
                            <div className="font-bold text-xs flex items-center gap-1.5">
                              <Sparkles className="w-3.5 h-3.5 text-cyan-400" />
                              <span>থিম ও লাইটিং সিলেক্টর</span>
                            </div>
                            <div className={`text-[10px] ${isDark ? "text-slate-400" : "text-slate-500"} mt-0.5`}>
                              {MANAGEMENT_THEMES.length} টি প্রিমিয়াম থিম
                            </div>
                          </div>
                          
                          {/* Day / Night Toggle Pill */}
                          <div className={`p-0.5 rounded-lg border flex items-center ${isDark ? "bg-black/40 border-white/10" : "bg-slate-100 border-slate-200"}`}>
                            <button
                              type="button"
                              onClick={() => {
                                setIsDark(false);
                                saveManagementThemeConfig(
                                  activeMgmtTheme.isDark ? "luxury_pearl_light" : activeMgmtTheme.id,
                                  undefined, undefined, undefined, false
                                );
                                showToast("☀️ ফ্রেশ ডে মোড (লাইট থিম) সক্রিয় হয়েছে");
                              }}
                            ...
                            <button
                              ...
                              onClick={() => {
                                setIsDark(true);
                                saveManagementThemeConfig(
                                  !activeMgmtTheme.isDark ? "cyber_obsidian" : activeMgmtTheme.id,
                                  undefined, undefined, undefined, true
                                );
                                showToast("🌙 প্রিমিয়াম নাইট মোড (ডার্ক থিম) সক্রিয় হয়েছে");
                              }}
                              className={`px-2 py-1 rounded-md text-[10px] font-bold flex items-center gap-1 transition-all ${
                                isDark
                        ...
                        <div className="mb-2.5">
                          <div className="text-[10px] font-bold text-amber-500 flex items-center gap-1 mb-1.5 uppercase tracking-wider px-1">
                            <Sun className="w-3 h-3" />
                            <span>ডে মোড থিমসমূহ (Day Mode - 3 Options)</span>
                          </div>
                          <div className="space-y-1">
                            {MANAGEMENT_THEMES.filter((t) => !t.isDark).map((t) => {
                              const isCurrent = activeMgmtTheme.id === t.id && !isDark;
                              return (
                                <button
                                  key={t.id}
                                  onClick={() => {
                                    setIsDark(false);
                                    saveManagementThemeConfig(t.id, undefined, undefined, undefined, false);
                                    showToast(`☀️ ${t.name} থিম সেট করা হয়েছে`);
                                    setIsThemeMenuOpen(false);
                                  }}
                                  className={`w-full text-left p-2 rounded-xl flex items-center justify-between border transition-all text-xs ${
                                    isCurrent
                                      ? "bg-cyan-500/15 border-cyan-500/50 shadow-sm ring-1 ring-cyan-500/30 font-bold"
                                      : isDark
                                ...
                                >
                                  <div className="flex items-center gap-2">
                                    ...
                                    />
                                    <div>
                                      <div className={isCurrent ? "text-cyan-400" : ""}>{t.name}</div>
                        ...
                        {/* 🌙 Night Mode Section */}
                        <div>
                          <div className="text-[10px] font-bold text-indigo-400 flex items-center gap-1 mb-1.5 uppercase tracking-wider px-1">
                            <Moon className="w-3 h-3" />
                            <span>নাইট মোড থিমসমূহ (Night Mode - 9 Options)</span>
                          </div>
                            ...
                            {MANAGEMENT_THEMES.filter((t) => t.isDark).map((t) => {
                              ...
                              return (
                                <button
                                  ...
                                  onClick={() => {
                                    setIsDark(true);
                                    saveManagementThemeConfig(t.id, undefined, undefined, undefined, true);
                                    showToast(`🌙 ${t.name} থিম সেট করা হয়েছে`);
                                    setIsThemeMenuOpen(false);
                                  }}
                                  className={`w-full text-left p-2 rounded-xl flex items-center justify-between border transition-all text-xs ${
                                    isCurrent
                        ...
                        <div className={`pt-2 mt-2 border-t flex items-center justify-between ${isDark ? "border-white/10" : "border-slate-200"}`}>
                          <button
                            type="button"
                            onClick={() => {
                              setIsThemeMenuOpen(false);
                              ...
                              setTimeout(() => {
                                const el = document.getElementById("theme-settings-anchor");
                          ...
                          >
                            <span>অ্যাডভান্সড থিম ও কালার কাস্টমাইজেশন ⚙️</span>
                          </button>
                          <button
                            type="button"
                            onClick={() => setIsThemeMenuOpen(false)}
                            className="p-1 rounded-lg text-slate-400 hover:text-white transition-colors text-xs"
                          >
                ...
                {/* Direct 1-Click Dark/Light Toggle */}
                <button
                  onClick={() => {
                    const nextDark = !isDark;
                    setIsDark(nextDark);
                    saveManagementThemeConfig(undefined, undefined, undefined, undefined, nextDark);
                    showToast(nextDark ? "🌙 ডার্ক মোড সক্রিয় করা হয়েছে" : "☀️ ফ্রেশ ডে মোড সক্রিয় করা হয়েছে");
                  }}
                  className={`px-2.5 py-1.5 rounded-lg text-xs flex items-center gap-1.5 border transition-all ${
                    isDark
